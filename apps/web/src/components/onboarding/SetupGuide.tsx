      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b bg-muted/30">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {allDone ? 'You have completed all getting started steps.' : `${done} of ${total} steps done`}
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {onStartTour && !state.tourDone && (
      ...
      {allDone && (
        <div className="px-5 py-4 bg-green-50 dark:bg-green-950/20 border-t border-green-100 dark:border-green-900/30 text-center">
          <p className="text-sm text-green-700 dark:text-green-400 font-medium">
            All done! You can dismiss this guide or keep it as a reference.
          </p>
        </div>
      )}
    </div>
