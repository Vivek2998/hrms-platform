...
function buildEmailHtml(data: EmailJobData): { subject: string; html: string } {
  switch (data.type) {
    ...
    case "PAYSLIP":
      return {
        subject: `Your Payslip for ${data.month}/${data.year} is Ready`,
        html: `
          <h2>Hi ${data.firstName},</h2>
          <p>Your payslip for ${data.month}/${data.year} is now available.</p>
...
export const emailWorker = new Worker<EmailJobData>(
  ...
  async (job) => {
    ...
    job.log(`Email sent: ${subject} → ${job.data.to}`);
  },
  {
